...
interface StarsProps {
  ...
  color?: string;
  showNum?: boolean;
  /** When true, each star is tappable to set a whole-number rating. */
  interactive?: boolean;
  /** Called with the new rating (1-5) when a star is tapped. */
...
export function Stars({
  ...
  for (let i = 0; i < 5; i++) {
    ...
    let starEl: React.ReactNode;
    if (fill === 0.5) {
      // For half stars, use View with clipPath simulation via nested views
      starEl = (
        <View style={{ width: size, height: size, marginRight: 2, position: 'relative' }}>
          {/* Filled half */}
      ...
      starEl = (
        <Svg width={size} height={size} viewBox="0 0 20 20" style={{ marginRight: 2 }}>
          <Path
            d="M9.05 2.93c.3-.92 1.6-.92 1.9 0l1.07 3.29a1 1 0 00.95.69h3.46c.97 0 1.37 1.24.59 1.81l-2.8 2.03a1 1 0 00-.37 1.12l1.07 3.29c.3.92-.75 1.69-1.54 1.12l-2.8-2.03a1 1 0 00-1.17 0l-2.8 2.03c-.78.57-1.84-.2-1.54-1.12l1.07-3.29a1 1 0 00-.36-1.12l-2.8-2.03c-.79-.57-.38-1.81.59-1.81h3.46a1 1 0 00.95-.69l1.07-3.29z"
    ...
    stars.push(
      interactive ? (
        ...
        <React.Fragment key={i}>{starEl}</React.Fragment>
      )
    );
  }
